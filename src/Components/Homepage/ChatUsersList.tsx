  ...
  return (
    <>
      ...
      >
          ...
          <Flex flexDir="column" flex="1" overflowY="scroll" gap="2">
            {rooms.length >= 1 ? (
              rooms
                ...
                .map((room) => (
                  <ChatUsersListItem key={room.room} item={room} />
                ))
            ) : (
              <Text>
                You have no conversations. Start by clicking on the add button,
                and add someone to send him a message.
              </Text>
            )}
          </Flex>
        )}
        ...
        <AnimatePresence>
          {isContactResearchActive && <SearchUsersList key={"search"} />}
        </AnimatePresence>
      </Flex>
    </>
  );
